export const productValidation = (product: { title: string; description: string; image: string; price: string }) => {
  ...
  if (!product.title.trim()||product.title.length<5||product.title.length>80) {
    errors.title= "Title should be between 5 and 80 characters long.\n";
   
  }
  if (!product.description.trim()||product.description.length<5||product.description.length>900) {
    errors.description="Description should be between 5 and 900 characters long.";
  }
